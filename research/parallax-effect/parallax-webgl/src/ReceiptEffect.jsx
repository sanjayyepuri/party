...
class CustomReceiptEffectImpl extends Effect {
  ...
  update(_renderer, _inputBuffer, _deltaTime) {
    // Uniform is updated via ref in useEffect
  }
}

const CustomReceiptEffect = wrapEffect(CustomReceiptEffectImpl);

export function ReceiptEffect({ pixelSize = 8.0 }) {
  const effectRef = useRef();

  useEffect(() => {
    if (effectRef.current?.uniforms?.has("pixelSize")) {
      effectRef.current.uniforms.get("pixelSize").value = pixelSize;
    }
  }, [pixelSize]);

  return <CustomReceiptEffect ref={effectRef} pixelSize={pixelSize} />;
}
